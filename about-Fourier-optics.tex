...
% \oops{Define $ε$, $\epsilon_{0}$, and $μ_{0}$.}
...
\begin{examplebox}{Spherical wave.}
  ...
  \begin{align}
    \frac{∂u(\vec{r})}{∂x}
    % &= a\,\mathe^{\mathi\,k·\Norm*{\vec{r} - \vec{r}_{0}}}\,
    % \Paren*{
    %   \frac{\mathi\,k}{\Norm*{\vec{r} - \vec{r}_{0}}^{2}}
    %   - \frac{1}{\Norm*{\vec{r} - \vec{r}_{0}}^{3}}
    % }\,\bigl(x - x_{0}\bigr)\notag\\
    &= \Paren*{
...
%  σ = \sign(k_{z}) = \pm1
%\end{equation}
%with $k_{z} = \vec{k}·\vec{e}_{z}$ the algebraic projection of the wave vector
%$\vec{k}$ on the $z$-axis. We introduce $σ = \pm1$ to account for whether
...
\begin{equation}
  ...
  \mathe^{±\mathi\,2\,π\,\vec{x}·\vec{α}}\,\mathd^{2}x
  %= \frac{-\mathi}{ρ}\,\mathe^{\frac{\mathi\,π}{ρ}\,\Norm*{\vec{α}}^{2}}
...
\begin{equation}
  ...
  %≈ \mathe^{\mathi\,k\,z}\,\mathe^{\mathi\,2\,π\,\vec{α}·\vec{x}}
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=1,%grid lines/.style={solid}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Closest nodes on a grid.}
...
\begin{align}
  ...
  %\Abs*{r[\V{j} + δ\V{j}] - r[\V{j}]}\\
  %&\approx \max_{\substack{\V{j} \in Ω\\δ\V{j} \in δΩ}}
  %\Abs*{\Inner*{\frac{\partial r[\V{j}]}{\partial\V{j}}}{δ\V{j}}}\\
...
\end{equation}
% In other words, the grid cells, of size $δx$, at the edge of an aperture of
% diameter $D$ must be seen from a range $z$ with an angle that is smaller than
% the diffraction angle $λ/D$ by the aperture.
...
\begin{align}
  ...
  \Abs*{\phi[\V{j} + δ\V{j}] - \phi[\V{j}]} ≤ π
\end{align}
% \emph{Objective.} Compute: $\displaystyle
% \max_{\substack{\V{j} \in Ω\\δ\V{j} \in δΩ}}
...
\begin{figure}
  ...
  \begin{tikzpicture}[scale=1,%grid lines/.style={solid}
    ...
    \tikzmath{
      \gridstep=0.2;
      \gridsize=17;
      \Ax=\gridstep*(\gridsize - 1)/2;
      \Ay=\Ax;
      \Ar=sqrt(\Ax^2 + \Ay^2) + 0.3*\gridstep;
      \Bx=\Ax;
      \By=4*\gridstep;
      \Br=sqrt(\Bx^2 + \By^2) + 0.15*\gridstep; % med. radius
      \Cx=5*\gridstep;% offset of extreme nodes in min. case
      \Cy=1*\gridstep;% offset of extreme nodes in min. case
      \Cr=sqrt(\Cx^2 + \Cy^2) + 0.15*\gridstep; % min. radius
    ...
    \begin{scope}[xshift=-50mm]
      ...
      \draw[grid lines] (-\Ax,-\Ay) grid[step=\gridstep] (\Ax,\Ay);
      \draw (0,0) circle (\Ar);
      \draw[<-] (60:\Ar) -- (60:\Ar+0.4) -- ++(0.4,0) node [anchor=west] {$\min(1/λ,α_{\Tag{max}})$};
    \end{scope}
    % Case B:
    \begin{scope}
      \foreach \x in {-\Bx,+\Bx} {
   ...
   % Case C:
     \begin{scope}[xshift=40mm]
      ...
      \draw (\x + 0.7*\tick, -\Ay) node [anchor=west] {$-α_{\Tag{Nyquist}}$};
      \draw (\x + 0.7*\tick,    0) node [anchor=west] {$0$};
      \draw (\x + 0.7*\tick, +\Ay) node [anchor=west] {$+α_{\Tag{Nyquist}}$};
    \end{scope}
  \end{tikzpicture}
  \caption{Discrete spatial frequencies (indicated by colored circles) where
    the phase wrapping is the most severe for the non-paraxial propagation
    transfer function. The $N\times N$ grid indicates the positions of the
    discrete frequencies. The circle indicates the smallest of the cutoff
    frequency of the angular spectrum and of the evanescent waves limit at
    $1/λ$ for the 3 possible cases.}
