...
\newcommand{\NyquistFreq}{α_{\Tag{Nyquist}}}
\newcommand{\CutoffFreq}{α_{\Tag{max}}}
...
%$\V{k}$ on the $z$-axis. We introduce $σ = \pm1$ to account for whether
...
\begin{equation}
  δΞ = \Brace*{(±δα,0),(0,±δα)}
\end{equation}
the offsets to the closest spatial frequencies (the blue dots in
Fig.~\ref{fig:closest-nodes}) and with:
\begin{equation}
  Ξ = \Brace*{
    \V{α} \in \Set{R}^{2} \Given \Norm{\V{α}} < \CutoffFreq
    \text{\ and\ } \Norm{\V{α}}_{\infty} ≤ \NyquistFreq
  }\\
\end{equation}
the domain where spatial frequencies are below the cutoff frequency
$\CutoffFreq$ of the angular spectrum $\FT{u}_{z}$ and the Nyquist frequency:
\begin{equation}
  \label{eq:Nyquist-frequency}
  \NyquistFreq = \frac{1}{2}\,N\,δα = \frac{1}{2\,δx}
\end{equation}
for a $N\times N$ grid of discrete spatial frequencies. Note that
$\CutoffFreq$ may be infinite.

Assuming the spectral sampling step $δα = 1/(N\,δx)$ is small enough, the
maximal absolute phase difference value can be estimated following the same
steps as in
section~\ref{sec:Rayleigh-Sommerfeld-diffraction:sampling-constraint}:
\begin{align}
  δ\phi_{\Tag{max}}
  ...
  &= k\,\Abs*{z}\,λ^{2}\,\max_{\substack{\V{α} \in Ξ\\δ\V{α} \in δΞ}}
  \Abs*{\frac{\Inner*{\V{α}}{δ\V{α}}}{\sqrt{1 - \Norm{λ\,\V{α}}^{2}}}}\\
  &= k\,\Abs*{z}\,λ^{2}\,\max_{\V{α} \in Ξ}
    \frac{\max_{δ\V{α} \in δΞ}\Abs*{\Inner*{\V{α}}{δ\V{α}}}}
    {\Abs*{\sqrt{1 - \Norm{λ\,\V{α}}^{2}}}}\\
  &= k\,\Abs*{z}\,λ^{2}\,δα\,\max_{\V{α} \in Ξ}
    \frac{\Norm*{\V{α}}_{\infty}}
    {\Abs*{\sqrt{1 - \Norm{λ\,\V{α}}^{2}}}}.
    \label{eq:δϕ_max:nonparaxial}
\end{align}
Introducing $L = \max_{\V{α} \in Ξ}\Norm{\V{α}} ≤ N/\sqrt{2}$
and noting that $L ≤ N/\sqrt{2}$ due to the finite number of samples along a
lateral dimension, there are different cases to consider:
\begin{itemize}
\item If
      $\max_{\V{α} \in Ξ}\Norm{\V{α}} = \min(\CutoffFreq,\sqrt{2}\,\NyquistFreq) ≥ 1/λ$,
      then the sampled spatial frequencies include evanescent waves and
      $\sqrt{1 - \Norm{λ\,\V{α}}^{2}}$ may be arbitrarily close to zero for
      $\Norm*{\V{α}}_{\infty} > 0$. Hence, the condition in
      Eq.~\eqref{eq:phase-sampling-condition} cannot hold and phase wrapping is
      irreversible.
\item Otherwise, maximizing the right-hand-side in
      Eq.~\eqref{eq:δϕ_max:nonparaxial} amounts to finding the frequencies in
      $Ξ$ such that $\Norm*{\V{α}}_{\infty}$ and $\Norm*{\V{α}}$ are both
      maximized. The 3 possibilities are illustrated by
      Fig.~\ref{fig:phase-wrapping-non-paraxial-transfer}. If
      $\sqrt{2}\,\NyquistFreq < \min(1/λ,\CutoffFreq)$, then the worst
      spatial frequencies are indicated by blue dots in the leftmost panel of
      ...
      \begin{equation}
        ...
        \begin{cases}
          \displaystyle
          \frac{2\,λ\,\Abs*{z}\,δα\,\CutoffFreq}{\sqrt{1 - λ^{2}\,\CutoffFreq^{2}}}
          =\frac{2\,λ\,\Abs*{z}\,\CutoffFreq}{N\,δx\,\sqrt{1 - λ^{2}\,\CutoffFreq^{2}}}
          ≤ 1\\[2ex]
          \displaystyle
          \CutoffFreq ≤ \NyquistFreq = \frac{N}{2\,δx}
        \end{cases}
      \end{equation}
\end{itemize}
...
\begin{figure}
  ...
  \caption{Spatial frequencies (indicated by colored circles) where the phase
    ...
    sampled by the DFT and the circle indicates the cutoff frequency
    $\CutoffFreq$ of the angular spectrum assumed to be less the evanescent
    waves limit at $1/λ$.}
...
In the paraxial conditions, the propagation transfer function is given by Eq.~\eqref{eq:paraxial-transfer-function}
which correspond to frequencies much smaller than $1/λ$:
\begin{equation}
...
\begin{align}
  ...
  &=\max_{\substack{\V{α} \in Ξ\\δ\V{α} \in δΞ}}
  \Abs*{\phi(\V{α} + δ\V{α}) - \phi(\V{α})} \\
  &= π\,λ\,\Abs*{z}\,\max_{\substack{\V{α} \in Ξ\\δ\V{α} \in δΞ}}\Norm*{\V{α}}^{2}\\
